...
export default function ImageOptimizer() {
        ...
        canvas.toBlob(
          (blob) => {
            if (blob) {
              ...
              // Approx 0.2g CO2 per 1MB transfer
              const co2SavedGrams = Number(((savedBytes / (1024 * 1024)) * 0.2).toFixed(4));
  ...
  const handleQualityChange = (newQual) => {
    setQuality(newQual);
    if (originalFile) {
      compressImage(originalFile, newQual);
    }
  };
  ...
  const handleDownload = () => {
    ...
    nameParts.pop();
    a.download = `${nameParts.join('.')}_optimized.webp`;
    a.click();
  };
    ...
    <div className="glass-panel p-6 rounded-3xl border border-emerald-900/40 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-emerald-950/60 pb-4">
        <div>
          <h3 className="text-lg font-bold text-white flex items-center gap-2">
            ...
            Client-Side Next-Gen Image Compressor (WebP)
          </h3>
          <p className="text-xs text-slate-400 mt-0.5">
            Convert heavy PNG/JPG assets directly in your browser to cut image transfer weight by up to 70%
          </p>
        </div>

        <span className="text-[11px] font-mono px-2.5 py-1 rounded-full bg-emerald-500/10 text-emerald-300 border border-emerald-500/20 w-fit">
      ...
      {!previewUrl ? (
        <label className="border-2 border-dashed border-emerald-800/60 hover:border-emerald-400/80 rounded-2xl p-8 flex flex-col items-center justify-center cursor-pointer transition-all bg-emerald-950/20 hover:bg-emerald-950/40 text-center group">
          ...
          <span className="text-xs text-slate-400 mt-1">Supports PNG, JPEG, GIF (Outputs lean WebP)</span>
        </label>
      ) : (
        <div className="space-y-6">
          ...
          >
            ...
            <span>Download Lean WebP Image</span>
          </button>
        </div>
      )}
    </div>
